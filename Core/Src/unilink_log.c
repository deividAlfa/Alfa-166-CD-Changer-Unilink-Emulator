...
char hex2ascii(uint8_t hex) {
    hex &= 0xF;
    return (hex + (hex<10 ? '0' : '7'));
}

uint8_t bcd2hex(uint8_t bcd) {
    uint8_t tmp;
    if(bcd>0x99)
        return bcd;             // Hack to allow hex digits in Aux and BT modes
    tmp = ((bcd & 0xF0) >> 4) * 10;
    tmp += (bcd & 0x0F);
    return tmp;
}
...
uint8_t hex2bcd(uint8_t hex) {
    uint8_t tmp;
    if(hex>99)
        return hex;             // Hack to allow hex digits in Aux and BT modes
    tmp = (hex / 10) << 4;
    tmp += (hex % 10);
    return tmp;
}
...
void unilinkLogUpdate(unilink_SPImode_t mode) {
    ...
    if (mode == mode_rx) {
        ...
        Log.size = unilink.rxSize;
    }
    else {
        s = (uint8_t*) unilink.txData;
        Log.size = unilink.txSize;
    }
...
void unilinkLogShow(void) {
    ...
    if (!Log.available) {
        switch (slaveBreak.break_state) {
            ...
            case break_wait_data_high: {
                if (ReadPin(UNILINK_DATA) == 1) {                // Wait for DATA high for 3ms
            ...
            case break_wait_data_setlow: {
                if (ReadPin(UNILINK_DATA) == 0) {                // If data goes low within time
                    slaveBreak.dataTime = 2;                // Load timer with 2ms
                    slaveBreak.break_state = break_wait_data_keeplow;                // Break started
                }
                    ...
                    putString("B? ");                // Slave could have sent a break, but didn't
                    slaveBreak.break_str = 1;
                }
                break;
            }
            case break_wait_data_keeplow: {
                if (!slaveBreak.dataTime) {                // Timer expired, data low end, break executed
                    slaveBreak.break_state = break_wait_data_low;                // Reset status
                    if (slaveBreak.break_str) {                // If we had previous break attempts, make new line
                        putString("\r\n");
          ...
          unlinkLogTimestamp();
                ...
                else if (ReadPin(UNILINK_DATA) == 1) {
                    slaveBreak.break_state = break_wait_data_low;                // If DATA goes high before the timer expires, invalid break
                }
            }
            default:
                break;
    ...
    if (slaveBreak.break_str) {                // If we had break attempts but master sent a new frame, clear the line
        putString("\r\n");
        slaveBreak.break_str = 0;
    }
#endif
    ...
    uint8_t dst = Log.data[dst_addr];
    uint8_t group_id = dst & 0xF0;
    if (dst == addr_broadcast || group_id == addr_reset) {                // From master to slave ID or broadcast
        str[i++] = '<';
        str[i++] = '<';
        str[i++] = ' ';
    }
    else if (group_id == addr_master || dst == addr_display
        || dst == addr_dsp) {                // From slave to master
        str[i++] = ' ';
        str[i++] = ' ';
        str[i++] = '#';
    }
...
#endif
        str[i++] = hex2ascii((Log.data[count]) >> 4);
        str[i++] = hex2ascii((Log.data[count]) & 0x0f);
        count++;
#ifdef UNILINK_LOG_DETAILED
...
#ifdef UNILINK_LOG_DETAILED
        if ((count == parity1) || (size > unilink_short && count == d1)
            ||                // Print fields between brackets [x]
  ...
  str[i++]='\n';
    ...
    switch (Log.data[cmd1]) {
        case cmd_status:
            putString("STATUS: ");

            switch (Log.data[cmd2]) {
                case 0x00:
                    putString("Playing\r\n");
                    break;
        ...
        case cmd_repeat:
            ...
            putString(Log.data[cmd2] & 0x10 ? "ON" : "OFF");
            putString("\r\n");
            break;
        ...
        case cmd_cfgchange: {
            ...
            if (Log.data[d1] & 0x80) {
                putString(" Mute:");
                putString(Log.data[d1] & 0x40 ? "ON" : "OFF");
            }
            if (Log.data[d1] & 0x20) {
                putString(" Loudness:");
                putString(Log.data[d1] & 0x10 ? "ON" : "OFF");
            }
            if (Log.data[d2] & 0x80) {
                putString(" Phone mute:");
                putString(Log.data[d2] & 0x40 ? "ON" : "OFF");
            }
            putString("\r\n");
            break;
        }
        case cmd_switch:
            putString("Switch to ");
            putString(Log.data[cmd2] == 0x10 ? "CD?" : "???");
            putString("\r\n");
            break;
        ...
        case cmd_time: {
            ...
            putString("Disc:");
            s[0] = h2a[Log.data[d4] >> 4];
            putString(s);
            putString(" Track:");
            s[0] =
                (Log.data[d1] >> 4) == 0xF ?
                    '0' : h2a[Log.data[d1] >> 4];
            s[1] = h2a[Log.data[d1] & 0xF];
            s[2] = ' ';
            putString(s);
            ...
            s[0] =
                (Log.data[d2] >> 4) == 0xF ?
                    '0' : h2a[Log.data[d2] >> 4];
            s[1] = h2a[Log.data[d2] & 0xF];
            s[2] = ':';
            putString(s);

            s[0] = h2a[Log.data[d3] >> 4];
            s[1] = h2a[Log.data[d3] & 0xF];
            s[2] = '\r';
            s[3] = '\n';
            putString(s);
            break;
        ...
        case cmd_magazine:
            putString("MAGAZINE INFO: ");
            putString("CD1:");
            putString(Log.data[cmd2] & 0x10 ? "Y" : "N");
            putString(" CD2:");
            putString(Log.data[cmd2] & 0x20 ? "Y" : "N");
            putString(" CD3:");
            putString(Log.data[cmd2] & 0x40 ? "Y" : "N");
            putString(" CD4:");
            putString(Log.data[cmd2] & 0x80 ? "Y" : "N");
            putString(" CD5:");
            putString(Log.data[cmd2] & 0x01 ? "Y" : "N");
            putString(" CD6:");
            putString(Log.data[cmd2] & 0x02 ? "Y" : "N");
            putString("\r\n");
            break;
        ...
        case cmd_discinfo: {
            ...
            putString("Tracks:");
            s[0] = h2a[Log.data[d1] >> 4];
            s[1] = h2a[Log.data[d1] & 0xF];
            s[2] = ' ';
            putString(s);

            s[0] = h2a[Log.data[d2] >> 4];
            s[1] = h2a[Log.data[d2] & 0xF];
            s[2] = 'm';
            s[3] = ':';
            putString(s);
        ...
        case cmd_source: {
            ...
            putString(s);
            putString(" to ");
            s[0] = h2a[Log.data[cmd2] >> 4];
            s[1] = h2a[Log.data[cmd2] & 0xF];
            putString(s);
            putString("\r\n");
            break;
        }
